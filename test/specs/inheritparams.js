describe("@inheritparams tag", function(){
    var plugin = require('addons/inheritparams');
    var parser = new (require("jsdoc/src/parser")).Parser();
    require('jsdoc/plugins').installPlugins(['addons/inheritparams'], parser);

    var docSet = jasmine.getDocSetFromFile('addons/test/fixtures/inheritparams.js', parser);

    plugin.handlers.processingComplete(docSet);

    var shape  = docSet.getByLongname('Shape')[0],
